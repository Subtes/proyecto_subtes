 ...
 */
Doors_Controller::Doors_Controller(SubteStatus *subte, Panel_Buttons3 * leftPanel,Panel_Buttons3 * rightPanel,QDial * dial)
{
    m_subte = subte;
...
void Doors_Controller::switchPanels(int value){
    switch (value) {
        ...
        case LEFT:
            ...
            updatePanel(m_subte->getLeftDoor(),m_leftPanel);
            turnOffPanel(m_rightPanel);
            break;

        case RIGHT:
            turnOnPanel(m_rightPanel);
            updatePanel(m_subte->getRightDoor(),m_rightPanel);
            turnOffPanel(m_leftPanel);
            break;
    }
}
...
void Doors_Controller::openLeftDoors(){
    m_subte->changeLeftDoor(SubteStatus::DOORS_OPENED);
    updatePanel(SubteStatus::DOORS_OPENED,m_leftPanel);
}
...
void Doors_Controller::closeLeftDoors(){
    m_subte->changeLeftDoor(SubteStatus::DOORS_CLOSED);
    updatePanel(SubteStatus::DOORS_CLOSED,m_leftPanel);
}
...
void Doors_Controller::openRightDoors(){
    m_subte->changeRightDoor(SubteStatus::DOORS_OPENED);
...
void Doors_Controller::closeRightDoors(){
    m_subte->changeRightDoor(SubteStatus::DOORS_CLOSED);
    updatePanel(SubteStatus::DOORS_CLOSED,m_rightPanel);
}
