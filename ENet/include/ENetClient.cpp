...
#pragma region Cambio de Clave
      if (partes[0] == "valClave")
      {
        if (partes.size() == 3) partes.push_back("");
        if (partes.size()!=4 && partes.size()!=5)
        {
        ...
        {
          DicClave* unaClave = new DicClave(partes[1],partes[2], partes.size() == 4 ? "" : partes[3]);
          std::string valor = partes.size() == 4 ? partes[3] : partes[4];

          Claves.AgregarClave(*unaClave, valor);
          if (!ExisteCliente(unaClave->Host) || _dicClientes[unaClave->Host])
          {
            if (unaClave->SubClave == "" && OnCambioValClave != nullptr)
              OnCambioValClave(unaClave->Host, unaClave->Clave, valor);
            if (unaClave->SubClave != "" && OnCambioValClaves != nullptr)
              OnCambioValClaves(unaClave->Host, unaClave->Clave, unaClave->SubClave, valor);
          }
        }
      }
#pragma endregion 
...
bool ENetClient::Suscribirse(std::string unCliente, std::string unaClave, std::string unaSubClave)
{
	return EnviarMensaje(1, "suscripPeer||" + unCliente + "||" + unaClave + "||" + unaSubClave) ;
}
...
bool ENetClient::DeSuscribirse(std::string unCliente, std::string unaClave, std::string unaSubClave)
{
	return EnviarMensaje(1, "dsuscripPeer||" + unCliente + "||" + unaClave + "||" + unaSubClave);
}

bool ENetClient::CambiarValorClave(std::string unaClave, std::string unNuevoValor)
{
  Claves.AgregarClave(Nombre, unaClave, unNuevoValor);
  return EnviarMensaje(1, "valClave||" + unaClave + "||" + unNuevoValor);
}

bool ENetClient::CambiarValorClave(std::string unaClave, std::string unaSubClave, std::string unNuevoValor)
{
  Claves.AgregarClave(Nombre, unaClave, unaSubClave, unNuevoValor);
  return EnviarMensaje(1, "valClave||" + unaClave + "||" + unaSubClave + "||" + unNuevoValor);
}
